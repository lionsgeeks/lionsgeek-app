            ...
            <div className="space-y-6">
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="no_diploma" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                            <TransText en="No diploma" fr="Je n'ai aucun diplôme" ar="ليس لدي أي شهادة" />
                        </option>
                        <option value="baccalaureate" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                            <TransText en="Baccalaureate" fr="Baccalauréat" ar="البكالوريا" />
                        </option>
                        <option value="technician" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                            <TransText en="Technician Diploma" fr="Diplôme Technicien (sans bac)" ar="شهادة التكوين المهني" />
                        </option>
                        <option value="deug_dut_dts_bts" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                            DEUG / DUT / DTS / BTS
                        </option>
                        <option value="licence" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                            <TransText en="Bachelor / Licence" fr="Licence / Licence Pro / Bachelor" ar="الإجازة" />
                        </option>
                        <option value="master" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                            <TransText en="Master / Engineer" fr="Master / Ingénieur / Diplôme équivalent" ar="الماجستير / مهندس" />
                        </option>
                        <option value="doctorate" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="job_seeking" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                            <TransText en="Looking for opportunities" fr="En recherche d'opportunité" ar="أبحث عن فرص" />
                        </option>
                        <option value="student" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                        ...
                        <option value="freelancer" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                            <TransText en="Freelancer" fr="Freelancer" ar="عامل حر" />
                        </option>
                        <option value="apprenticeship" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                            <TransText en="In apprenticeship" fr="En apprentissage" ar="في التدريب المهني" />
                        </option>
                        <option value="internship" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                            <TransText en="In internship" fr="En stage en entreprise" ar="في تدريب بالشركة" />
                        </option>
                        <option value="entrepreneur" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                ...
                {data.has_referring_organization === 'yes' && (
                    <div>
                        ...
                        >
                            ...
                            <option value="anapec" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                                الوكالة الوطنية لإنعاش التشغيل والكفاءات (أنابيك) - Anapec
                            </option>
                            <option value="heure_joyeuse" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                                Heure Joyeuse / جمعية ساعة الفرح
                            </option>
                            <option value="fondation_jadara" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                                Fondation Jadara / مؤسسة جدارة
                            </option>
                            <option value="plateforme_ain_sbaa" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                                Plateforme des Jeunes Ain Sbaa / منصة شباب عين السبع
                            </option>
                            <option value="plateforme_hay_mohammadi" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                                Plateforme des Jeunes Hay Mohammadi / منصة شباب الحي المحمدي
                            </option>
                            <option value="plateforme_roches_noires" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                                Plateforme des Jeunes Roches Noires / منصة شباب الصخور السوداء
                            </option>
                            <option value="autre_plateforme" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
                                Autre Plateforme des Jeunes / منصة شباب أخرى
                            </option>
                            <option value="autre_association" className={darkMode ? 'bg-gray-700' : 'bg-white'}>
